...
float average_list(vector<float> list){           // Het gemiddelde uitrekenen van de een lijst
    float total = list[0];
...
float average(vector<vector<float>> list){
    vector<float> sum = {};
    for (int x = 0; x < list.size(); ++x){      // lijsten individualiseren
         float av = average_list(list[x]);
         sum.insert(sum.end(), av);
    }
    float total_av = average_list(sum); // het gemiddelde van alle lijsten optellen
    return total_av;
}


void drie_zes(){
    vector<vector<float>> random = {  {7, 82, 52, 96, 65, 33, 16, 23,18, 3, 6},           // willekeurig gekozen nummers
